(() => {
  ...
  // ---------------------------------------------------------
  // API connection status (GET / — the only endpoint the
  // backend exposes besides /predict, so it doubles as a
  // liveness check)
  // ---------------------------------------------------------
  if (apiDocsLink) apiDocsLink.href = `${API_BASE}/docs`;

  function setApiStatus(state) {
    if (!apiStatusDot || !apiStatusText) return;
    apiStatusDot.classList.remove("is-checking", "is-connected", "is-down");
    apiStatusDot.classList.add(
      state === "connected" ? "is-connected" : state === "down" ? "is-down" : "is-checking"
    );
  ...
  async function checkApiStatus() {
    ...
    try {
      ...
      setApiStatus(res.ok ? "connected" : "down");
    } catch {
      setApiStatus("down");
    } finally {
      clearTimeout(timer);
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      renderError(
        ...
        `Couldn't connect to ${API_BASE}. The backend may be asleep or offline — try again in a moment.`
      );
      setApiStatus("down");
    } finally {
      setSubmitting(false);
